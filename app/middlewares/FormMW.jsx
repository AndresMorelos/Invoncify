  ...
  (action) => {
    switch (action.type) {
      case ACTION_TYPES.FORM_SAVE: {
        ...
        const { currentInvoiceData, recipient } = getInvoiceData(
          currentFormData,
          secretKey
        );
        // UPDATE DOC
      ...
      case ACTION_TYPES.FORM_ITEM_ADD: {
        return next({ ...action, payload: { id: uuidv4() } });
      }

      case ACTION_TYPES.FORM_ITEM_ADD_SUBITEM: {
        return next({
          ...action,
          payload: { parentItemId: action.payload, id: uuidv4() },
        });
      }
